...
// 圖文編輯的最小單位——比照 Internal-Pages（as-studio001/Internal-Pages，
// https://as-studio001.github.io/Internal-Pages/?case=laogu-fang）內文的圖文編輯模式：
// 一整段論述文字是「段落區塊」跟「圖片區塊」照作者排列順序交錯而成的一串序列，不是固定的
// 「大圖—文—圖—文」模板。圖片區塊可以放 1～2 張——2 張的話並排顯示（Internal-Pages 常見的
// 手法），是1還是2、放在哪兩段文字中間，每個章節自己決定，不用跟其他章節套同一套版型。
...
export type Chapter = {
  key: ChapterKey;
  title: string;
  // 「建築設計／模矩／構造／施工」用 blocks 決定圖文順序（見 NarrativeSection）；
  // 「組裝說明書」「基地」內容比較單純（一段文字＋PDF／地圖），沒有 blocks，還在用
  // text／images 這組舊欄位。
  blocks?: ContentBlock[];
  text?: string;
  images?: ProjectImage[];
  ...
  // 跟 blocks／images 分開存放——這批圖只在內頁展開後才看得到，不算進首頁雙欄的照片索引。
  video?: string; // 影片路徑（可選，每個章節都能放）
  pdf?: { url: string; label: string }; // 可下載檔案（目前只有「組裝說明書」會用到）
};

// 統一取得一個章節「拿來當照片」的清單——有 blocks 的章節（建築設計／模矩／構造／施工）
// 從 blocks 裡的圖片區塊自動攤平算出來；沒有 blocks 的章節（組裝說明書／基地）退回手動
// 維護的 images 欄位。PhotoStream.tsx（首頁雙欄照片索引）要拿某章節的照片一律呼叫這個
// 函式，不要直接讀 chapter.images——避免 blocks 內容跟 images 清單各自維護、兩邊對不上。
export function chapterPhotos(chapter: Chapter): ProjectImage[] {
  if (chapter.blocks) {
    return chapter.blocks
      .filter((b): b is Extract<ContentBlock, { type: "images" }> => b.type === "images")
      .flatMap((b) => b.images);
  }
    ...
    chapters: [
      {
        ...
        title: "建築設計",
        blocks: [
          { type: "paragraph", text: "〔佔位文字，抓版面用〕常民竹小屋的設計出發點，是重新思考「常民」與「構造」之間的距離——不是為了展示工法而蓋，而是回到最基本的生活尺度，思考人如何在竹構造的空間裡靜下來、待下來、慢慢住進去。整體配置刻意保留低矮、貼近地面的姿態，讓建築物退居成背景，把使用者的日常動作留在畫面中央，也呼應基地周邊聚落原有的生活紋理與尺度感，避免用一棟過於張揚的量體打斷既有的地景關係。" },
          {
            type: "images",
            images: [
          ...
          { type: "paragraph", text: "空間的開放程度、遮蔽與穿透的比例，都在設計初期反覆推敲，希望呈現一種介於室內與戶外之間、隨天氣與時間而變化的曖昧邊界。屋簷深淺、開口大小、動線轉折，每一個決定背後都對應著一段關於「怎麼住」的具體想像，而不只是造型上的考量，也希望觀者在走進這個空間時，能感受到設計者對日常生活細節的體貼。這段文字之後會替換成正式的策展論述。" },
        ],
        moreImages: [
      ...
      },
      {
        ...
        title: "模矩",
        blocks: [
          { type: "paragraph", text: "〔佔位文字，抓版面用〕模矩系統是整個構造邏輯的起點：以單根竹材的常見長度與直徑為基準，反推出一套可重複、可搭接的單元尺寸，讓每一個構件都能在不同的位置上互換使用，降低現場裁切與浪費，也讓非專業的參與者更容易理解整個系統的組成方式。這套模矩不只是結構計算的工具，也決定了立面的節奏與比例，形成一種可被閱讀的秩序感。" },
          { type: "paragraph", text: "從平面到剖面，模矩的網格貫穿整個設計，也成為施工階段溝通的共同語言——工班不需要逐一讀圖，只要理解模矩邏輯，就能判斷構件該落在哪裡，大幅降低溝通成本與現場出錯的機率。這種「先定規則、再長出形狀」的做法，是竹構造回應手工與效率之間拉扯的具體嘗試，之後會補上實際的模矩尺寸表與詳細圖說，讓論述更精確、更可被驗證。" },
          {
            type: "images",
            images: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: "paragraph", text: "〔佔位文字，抓版面用〕構造系統採用竹材本身的彈性與韌性作為設計出發點，主結構以桁架式的組合方式，將荷重層層分散到地面基礎，減少對單一構件強度的依賴，也讓整體結構在面對不規則外力時，能有一定程度的彈性緩衝而不至於瞬間破壞。接點的處理是整個構造裡最關鍵的一環，既要傳遞力量，也要保留竹材熱脹冷縮與含水率變化的空間，這也是竹構造與一般木構造、鋼構造在細部思考上最大的不同之處。" },
          { type: "paragraph", text: "為了兼顧結構安全與可拆解性，多數接點採用綁紮與栓接混合的方式，而非單一膠合固定，讓未來的維護、替換單一構件成為可能，也呼應「常民構造」該有的可修復性格——壞掉的地方可以局部處理，不用整棟拆除重建。整個結構系統的設計過程，也反覆與在地匠師討論、調整，融入他們長年累積的實作經驗。這部分之後會補上完整的結構計算報告與細部接點圖說，讓論述更精確、更貼近實際施作。" },
          {
            type: "images",
            images: [
      ...
      },
      {
        ...
        title: "施工",
        blocks: [
          { type: "paragraph", text: "〔佔位文字，抓版面用〕施工過程刻意保留了手工參與的比例，從竹材的選料、防腐處理，到現場組裝，都邀請在地工班與志工共同完成，而不是全部交給機具與預製構件。這個決定讓整體工期拉長，卻也讓構造本身多了一層「這裡的人親手蓋起來」的重量，也讓參與者對這個空間產生更深的認同與情感連結，這種參與感也是「常民構造」想要傳遞的核心精神之一。" },
          {
            type: "images",
            images: [
          ...
          { type: "paragraph", text: "現場搭建分成基礎放樣、主結構立起、次結構填充、屋面收頭四個階段，每個階段都留下影像與圖說紀錄，作為未來其他基地複製這套工法的參考。施工過程中遇到的天氣延誤、材料調度、人力調配等實際狀況，之後也會整理成完整的施工紀錄放進這個章節，讓經驗可以被傳承而不只是留在少數人的記憶裡，也讓下一次的搭建能少走一些彎路。" },
          {
            type: "images",
            images: [
    ...
    chapters: [
      {
        ...
        title: "建築設計",
        blocks: [
          { type: "paragraph", text: "〔佔位文字，抓版面用〕一籌的設計命題與常民竹小屋不同，它更像是一次對「臨時性」構造的重新詮釋——不追求恆久留存，而是思考一個構造物如何在短暫的使用週期裡，依然給人足夠的空間品質與情感重量。量體刻意輕盈、通透，讓光線與周邊環境自由穿越，也讓觀者不會把它誤認成一棟「正式的建築」，而是更接近一個暫時搭起的場景。" },
          {
            type: "images",
            images: [
          ...
          { type: "paragraph", text: "整體造型語彙走向更抽象、更幾何的方向，減少裝飾性的細節，讓竹材本身的紋理與結構邏輯成為唯一的表情。這種克制的態度，也是一籌與常民竹小屋在同一個展覽裡相互對話、彼此襯托的方式——一個貼地生根、一個輕盈暫留，兩種對「住」的想像並置出現，讓觀眾在兩件作品之間來回走動時，能感受到明顯的語氣落差。正式論述文字之後會補上。" },
        ],
        moreImages: [
      ...
      },
      {
        ...
        title: "模矩",
        blocks: [
          { type: "paragraph", text: "〔佔位文字，抓版面用〕一籌的模矩系統採用比常民竹小屋更小、更密的單元尺寸，讓構造在視覺上呈現細膩的紋理感，也更貼近臨時構造需要快速搭拆的操作邏輯。每個模矩單元的重量與尺寸都控制在單人可搬運的範圍內，讓搭建過程不需要依賴大型機具，也讓非專業人員能獨立完成部分組裝工作。" },
          { type: "paragraph", text: "小尺度模矩帶來的代價是接點數量大幅增加，因此設計團隊另外發展了一套快拆式的連接構件，讓組裝與拆卸都能在短時間內完成，同時維持結構的穩定性，也讓一籌具備在不同場地間反覆巡迴展出的可能性。這套模矩與快拆邏輯之間的取捨，是整個設計過程反覆測試最多次的環節，前後修改了好幾個版本才定案。這套快拆系統的細部圖說與構件清單，之後會整理進這個章節。" },
          {
            type: "images",
            images: [
      ...
      },
      {
        ...
        blocks: [
          {
            type: "images",
            images: [{ id: "b-structure-1", tone: "bg-neutral-300", caption: "構造 01" }],
          },
          { type: "paragraph", text: "〔佔位文字，抓版面用〕一籌的結構邏輯偏向輕構造（light structure）的思路，用大量小斷面的竹材互相支撐，取代少數幾根粗大構件的傳統做法，整體重量因此大幅降低，也讓構造本身帶有一種輕盈、幾乎懸浮的視覺效果，與常民竹小屋厚實穩重的量體形成對比，成為這次展覽裡兩種結構性格的直接對照。" },
          { type: "paragraph", text: "為了因應快速搭拆的需求，結構系統刻意避免使用不可逆的接合方式，改以夾具與繩索張力共同維持整體穩定，這也讓一籌得以在不同基地之間重複搭建、成為一個可移動的展示構造，而不是綁定在單一地點的固定建物。每一次搭建與拆卸，都會對構件進行檢查與微調，確保重複使用不影響結構安全，磨損較明顯的構件也會逐批汰換。結構驗算資料與接點測試紀錄，之後會補上。" },
          {
            type: "images",
            images: [{ id: "b-structure-2", tone: "bg-neutral-400", caption: "構造 02" }],
          },
        ],
        moreImages: [
      ...
      },
      {
        ...
        title: "施工",
        blocks: [
          { type: "paragraph", text: "〔佔位文字，抓版面用〕一籌的施工強調「速度」與「可重複」——整個搭建過程被拆解成標準化的動作序列，讓不熟悉竹構造的人員也能在短時間的教學後加入搭建工作，這也是這件作品作為教育示範的重要部分，希望降低一般人對竹構造「很難、很專業」的距離感，讓更多人願意親手嘗試。" },
          {
            type: "images",
            images: [{ id: "b-construction-1", tone: "bg-neutral-300", caption: "施工 01" }],
          },
          { type: "paragraph", text: "現場施工分成基座定位、模組組裝、整體校正、收邊固定四個階段，每個階段都控制在明確的時間範圍內，作為未來巡迴展示時工班排程的參考依據。每次搭建也會邀請幾位第一次接觸的參與者，藉此驗證教學流程是否真的容易上手，並蒐集他們的操作回饋來修正教學步驟。詳細的施工時間紀錄、人力配置與現場照片，之後會補齊放入這個章節，作為其他巡迴場地籌備時的參考基準。" },
          {
            type: "images",
            images: [{ id: "b-construction-2", tone: "bg-neutral-400", caption: "施工 02" }],
          },
        ],
        moreImages: [
